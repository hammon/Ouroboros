...
TermCloud = function(container,tab_id) {
...
TermCloud.prototype.draw = function(hash, options) {
/*
...
*/
  ...
  var minFreq = Ext.Array.min(values);
  var maxFreq = Ext.Array.max(values);
...
*/
  ...
  //
  html.push('<div class="term-cloud" style="overflow: auto">');
  
  hash.each(function(key,value,length) {
		var f = value;
		if(f > 0){
			var text = key + " (" + value + ")";
			var link = "javascript:cloudClick('" + text + "')";
			var freq = value;
			...
			html.push(self.escapeHtml(text));
			html.push('</span>');
			html.push('</a>');
			html.push(' ');
			rowInd+=1;
 ...
 /* 
	hash.each(function(pair) {
		...
		if(f > 0){
			var text = pair.key;
			var link = "javascript:gui.showSnippets(" + self.tab_id + ",'" + text + "')";
			var freq = pair.value;
			var size = TermCloud.MIN_UNIT_SIZE +
			Math.round((freq - minFreq) / range * TermCloud.RANGE_UNIT_SIZE);
			html.push('<a class="term-cloud-link" href="', (link || '#'), '" id="_tc_', id, '_', rowInd , '"');
...
*/  
	...
	var cloudHtml = html.join('');
	if(this.container){
		this.container.innerHTML = cloudHtml;
	}
	return cloudHtml;
